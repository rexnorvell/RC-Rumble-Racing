...
class Track:
    ...
    def __init__(self, name: TrackName) -> None:
        ...
        self.finish_line: pygame.Rect = self.FINISH_LINE_LOCATIONS[self.name]
        self.checkpoint_1: pygame.Rect = self.CHECKPOINT_LOCATIONS[self.name]

        self.track_image: pygame.Surface = pygame.image.load(self.TRACK_IMAGE_PATH.format(track_name=self.name.value, image_type=self.TRACK_IMAGE_TYPES[0])).convert()
        self.track_image = pygame.transform.scale(self.track_image,
                                                  (constants.WIDTH * self.TRACK_IMAGE_SCALE_FACTOR[self.name][0],
                                                   constants.HEIGHT * self.TRACK_IMAGE_SCALE_FACTOR[self.name][1]))

        track_image_mask: pygame.Surface = pygame.image.load(self.TRACK_IMAGE_PATH.format(track_name=self.name.value, image_type=self.TRACK_IMAGE_TYPES[1])).convert()
    ...
    def _create_playlist(self) -> list[tuple[str, int]]:
        ...
        playlist: list[tuple[str, int]] = [
            (constants.TRACK_AUDIO_PATH.format(track_name="general", song_type=self.TRACK_SONG_TYPES[0]), 0),
            (constants.TRACK_AUDIO_PATH.format(track_name="general", song_type=self.TRACK_SONG_TYPES[1]), 0),
            (constants.TRACK_AUDIO_PATH.format(track_name=self.name.value, song_type=self.TRACK_SONG_TYPES[2]), -1),
            (constants.TRACK_AUDIO_PATH.format(track_name="general", song_type=self.TRACK_SONG_TYPES[3]), 0),
            (constants.TRACK_AUDIO_PATH.format(track_name=self.name.value, song_type=self.TRACK_SONG_TYPES[4]), -1),
            (constants.TRACK_AUDIO_PATH.format(track_name="general", song_type=self.TRACK_SONG_TYPES[5]), 0)
        ]
        return playlist
